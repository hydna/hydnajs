...
function dataToUtf(data) {
  ...
  if (typeof data == "string") {
    ...
    for (var i = 0, l = data.length; i < l; i++) {
      ...
      result[i] = String.fromCharCode(buffer);
    }
  } else {
    throw new Error("Unsupported data type");
  }
...
if (AB_TRANSPORT_SUPPORT) {
  ...
  createFrame = createFrameBin;
} else {
  PAYLOAD_MAX_SIZE = 0x1554c;
  createFrame = createFrameUtf;
}
...
function OpenEvent(target, data, newid, oldid) {
  this.message = data;
  this.newid = newid;
...
function FlashSocketInterface() {
  ...
  BridgeSocket.prototype.onmessage = sockMessageUtfImpl;
