...
function CometSocket() {
  ...
  function handshakeHandler(self) {
    return function handler(event) {
      var target = event.target;
      target.removeEventListener("message", handler, false);
      target.addEventListener("message", receiveHandler(self), false);
  ...
  function BridgeSocket(url) {
    var self = this;
    var body = document.getElementsByTagName('body')[0];
    var name = TARGET_NAME.toLowerCase();
    var src;
    var target;

    src = url.protocol + "://" + url.authority + "/api/comet_handshake";

    this.id = "__" + name + (BridgeSocket.incr++) + "_comet_bridge__";

    this.elem = document.createElement('iframe');
    this.elem.setAttribute("id", this.id);
    this.elem.src = src;
    this.elem.onload = function () {
      self.bridge = self.elem.contentWindow;
      self.bridge.addEventListener("message", handshakeHandler(self), false);
      self.bridge.postMessage("ping", "*");
    };
