...
Channel.prototype._open = function(payload, newid) {
  ...
  this.onopen && this.onopen(payload, newid);
};
...
Connection.prototype.send = function(frame) {
  if (this.sock) {
    return this.sock.send(frame._raw);
  } else {
    return false;
  }
};
...
function dataToB64Payload(data) {
  var result;
  var view;
  var buffer;

  if (typeof data == "string") {
    result = unescape(encodeURIComponent(data));
  } else if (data instanceof ArrayBuffer ||
    ...
    for (var i = 0, l = data.length; i < l; i++) {
      buffer = (parseInt(data[i]) % 256 + 256) % 256;
...
function b64FrameSet(data) {
  ...
  if (payload) {
    ...
    if (payload.length > PAYLOAD_MAX_SIZE) {
      throw new Error("PAYLOAD_MAX_SIZE_ERR");
    }

    frame = frame.concat(payload);
  }

  this._raw = btoa(frame.join(""));
}
...
// 
//   view = new Array();
//   view[0] = String.fromCharCode(id >>> 24 & 0xff);
//   view[1] = String.fromCharCode(id >>> 16 & 0xff);
//   view[2] = String.fromCharCode(id >>> 8 & 0xff);
...
function FlashSocketInterface() {
  ...
  BridgeSocket.onready = function() {
    ...
    for (var id in all) {
      ...
      BridgeSocket.bridge.init(all[id].id, url);
    }
  };
...
function CometSocket() {
  ...
  // messages. We filter out the underlying BridgeSocket
  ...
  function BridgeSocket(url) {
    ...
    id = (BridgeSocket.incr++).toString(16);
  ...
  BridgeSocket.all = {};
  BridgeSocket.incr = 1;
  BridgeSocket.count = 0;


  BridgeSocket.prototype.messageHandler = function(event) {
