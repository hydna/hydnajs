...
var USE_BLOBS = ArrayBuffer && true;
...
messageParser = function(event) {
  ...
  if ((buffer = event.data) instanceof ArrayBuffer == false) {
    return this.onerror && this.onerror(new Error("ERR_UNSPORT_DATATYPE"));
  }
...
messageParser = function(event) {
  ...
  if (!data || !data.length) {
    return this.destroy(new Error("ERR_UNSPORT_DATATYPE"));
  }
...
function WebSocketInterface(url, C) {
  return function(url, C) {
    ...
    url = url.source.replace(/^https/, "wss")
                    .replace(/^http/, "ws");

    sock = new WebSocket(url, ["v1.ws.wink.org"]);
...
function FlashSocketInterface() {
  ...
  function BridgeSocket(url) {
    console.log("Create BridgeSocket");
  ...
  BridgeSocket.prototype.connect = function() {
    console.log("Call bridge connect");
    BridgeSocket.bridge.init(this.id, this.url.source);
    console.log("Call bridge connect end");
  };
