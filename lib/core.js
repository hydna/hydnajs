  ...
  switch (op) {
    ...
    case 0x1: // OPEN
      if (flag == OpenRequest.FLAG_REDIRECT) {
        ...
        try {
          ...
          extra = (payload.charCodeAt(1) << 16 |
                   payload.charCodeAt(2) << 8 |
                   payload.charCodeAt(3)) +
                   (payload.charCodeAt(0) << 24 >>> 0);
          if (payload.length > 4) {
            payload = decodeURIComponent(escape(payload.substr(4)));
          }
        } catch (err) {
          return this.destroy(new Error("ENCODING_ERR"));
        }
    ...
    case 0x2: // DATA
      if (payload && payload.length) {
        try {
          payload = flag & 1 ? atoutf(payload) : atobin(payload);
        } catch (err) {
          return this.destroy(new Error("ENCODING_ERR"));
        }
      }
