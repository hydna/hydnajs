  ...
  if (ptr === ALL_CHANNELS) {
    for (var chanptr in routes) {
      ...
      if (channel.readyState == Channel.OPEN && channel.readable) {
        channel._dispatchEvent(new MessageEvent(channel, flag, cloneData(data)));
      }
    }
  } else if ((channel = routes[ptr])) {
    if (channel.readyState == Channel.OPEN && channel.readable) {
      channel._dispatchEvent(new MessageEvent(channel, flag, data));
    }
  }
};
      ...
      if (ptr === ALL_CHANNELS) {
        for (var chanptr in routes) {
          ...
          if (channel.readyState == Channel.OPEN) {
            channel._dispatchEvent(new SignalEvent(channel, cloneData(data)));
          }
        }
      } else if ((channel = routes[ptr])) {
        if (channel.readyState == Channel.OPEN) {
          channel._dispatchEvent(new SignalEvent(channel, data));
        }
      }
      break;
