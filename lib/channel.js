...
Channel.prototype.connect = function(url, mode) {
  ...
  this.url = url.protocol + '://' + url.host + '/' + id;
...
Channel.prototype.send = function(data, priority) {
  ...
  frame = createFrame(this.id, OP_DATA, flag, data);
...
Channel.prototype.emit = function(data) {
  ...
  frame = createFrame(this.id, OP_SINGAL, FLAG_EMIT, data);
...
Channel.prototype._destroy = function(err) {
  ...
  if (!this._request) {
    frame = createFrame(this.id, OP_SIGNAL, FLAG_END, this._endsig);
    this._endsig = null;
...
Channel.prototype._open = function(payload, id) {
  var flushed = false;
  var event;
  var frame;
  var sig;
