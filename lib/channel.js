...
Channel.prototype._onopen = function(data) {
  ...
  this._dispatchEvent(new OpenEvent(this, data));
};
...
Channel.prototype._dispatchEvent = function(event) {
  var type;
  var callback;

  if (!event || typeof (type = event.type) !== 'string') {
    return;
  }

  try {
    if ((callback = this['on' + type])) {
